...
class Home extends Component {
  ...
  componentDidMount() {
    ApiService.Films().then(res => {
      this.setState({ films: res })
    }).catch(err => "error", "Erro na comunicação com a API ao tentar listar os autores");
    
    ApiService.ListaPeople().then(res => {
      this.setState({people : [...this.state.people, ...res.results]})
    }).catch(err => "error", "Erro na comunicação com a API ao tentar listar os autores");

    ApiService.Starships().then(res => {
      console.log(res)
    ...
    return (
      <Container fluid className="h-100 ">
        <Row md={12} className="h-100">  
          ...
          <Col md={1} className="first-box cor-primaria h-100-tablet">
            <div className="icon-m navbar-toggler collapsed">
              <span className="menu-icon__line"></span>
              <span className="menu-icon__line menu-icon__line-right"></span>
              <span className="menu-icon__line menu-icon__line-left"></span>
            </div>
            <div className="redes-socias h-100">
              <div className="redes-socias__list">
              <div className="redes-socias__item">
              ...
              <div className="redes-socias__item">
                <svg width="30px" height="30px" viewBox="0 0 320 512">
          ...
          <Col md={11} className="h-100-tablet">
            <Header />
            <Row md={12} className=" mt-4">
              <Col md={8}>
                <TituloPrincipal films={this.state.films} />
